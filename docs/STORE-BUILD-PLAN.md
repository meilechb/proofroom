...
- [x] S9.11 Shop grid search (`?q=` over title/description) and sort (`?sort=` featured/newest/price-asc/price-desc) via a no-JS GET form, with a distinct no-match state; related products already sit on product pages. _(Category/tag facets are a later refinement.)_
...
- [~] S28.7 Security review of the store diff (ongoing, per-phase). Found + fixed across the build: the download route served the **watermarked** `preview_url` for web/standard tiers (now rendered clean, S15.2); a crafted request could add **another studio's asset** to a collection (now `assertOwned` + studio-scoped joins); the orphan-blob sweep and storage metering ignored **digital files** in the private store (now counted/spared, S29.6); and the product/site **JSON-LD** injected studio text without escaping `<`, a `</script>` XSS vector on the public storefront (now a shared `JsonLd` that escapes it). Confirmed sound: multi-tenant `studio_id` scoping on every store/broadcast query, server-side price recomputation at both checkout routes + tiered bundles (client never supplies amounts), hashed grant/gift-card tokens, guarded gift-card draw-down, suppression/unsubscribe-aware broadcast sending with atomic per-recipient claims, HTML-escaped email bodies (`textToHtml`).
...
- [x] S32.3 Help docs / setup runbook — `docs/STORE-SETUP.md`: connect Stripe, open the store, add each product kind, set licences, delivery, manual mode, discounts/gift cards, analytics, and operator notes.
