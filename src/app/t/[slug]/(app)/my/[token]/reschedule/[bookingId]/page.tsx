...
import { bookingSettings, openWeekdays, withinChangeWindow, localDateISO, addDaysISO, BOOKING_HORIZON_DAYS } from "@/lib/booking-shared";
...
export default async function ReschedulePage({ params }: PageProps<"/t/[slug]/my/[token]/reschedule/[bookingId]">) {
  ...
  return (
    <div className="mx-auto w-full max-w-2xl px-5 sm:px-8 py-10">
      ...
      {canChange ? (
        <div className="mt-6">
          <RescheduleFlow slug={slug} token={token} bookingId={bookingId} currentLabel={currentLabel} minDateISO={minDateISO} maxDateISO={maxDateISO} openWeekdays={openWeekdays(settings)} blockedDates={settings.blockedDates} />
        </div>
      ) : (
