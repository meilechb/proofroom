...
export default async function LoginPage({ searchParams }: PageProps<"/login">) {
  ...
  const notice =
    sp.verified === "1" ? "Email verified. Sign in to continue."
    : sp.reset === "1" ? "Password updated. Sign in with your new password."
    : sp.deleted === "1" ? "Your account has been deleted. Thank you for using Proofroom."
    : sp.logged_out === "1" ? "You have been signed out."
    : null;
  const error =
    (typeof sp.error === "string" ? GOOGLE_ERRORS[sp.error] : null)
    ?? (sp.verify === "invalid" ? "That verification link is invalid or has expired. Sign in and request a new one from your account page." : null)
    ?? (sp.expired === "1" ? "Your session has expired. Sign in again to continue." : null);
